...
//! 8-bit folder icons: 8×8 one-colour pixel glyphs a tag can wear instead
...
impl Icon {
    ...
    /// Eight rows of eight; `#` is a lit pixel.
    pub fn rows(self) -> [&'static str; 8] {
        match self {
            Icon::Coffee => [
                "..#..#..", "...#..#.", "..#..#..", ".######.", ".#....##", ".#....##", ".######.",
                "..####..",
    ...
    /// Lit pixels as (x, y) in an 8×8 grid.
...
mod tests {
    ...
    fn every_icon_is_eight_by_eight_and_lit() {
        for icon in Icon::ALL {
            let rows = icon.rows();
            assert!(rows.iter().all(|r| r.len() == 8), "{icon:?}");
            assert!(icon.pixels().count() >= 8, "{icon:?} too faint");
