...
- Visual check without a human: `tools/xshot.py out.png [--keys ctrl+n --type 'text']`
  ...
  themes, theme, image, imgframe, imgalign, imgwidth, imgcaption, imgmenu, imgdrag, imgmove, fold, font, pairing, size, docksize, section, tagmenu, renametag, nav, togglebox, marker, measure, follow, icon, wait, exit; `;` separates steps — write `\;` inside text),
